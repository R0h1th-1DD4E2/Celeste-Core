module ram_5port(
    ...
    input write_enB,
    input read_enB,
    input [255:0] data_inB,
    output reg [255:0] data_outB,
    ...
    input write_enC,
    input read_enC,
    input [255:0] data_inC,
    output reg [255:0] data_outC,
    ...
    input write_enD,
    input read_enD,
    input [255:0] data_inD,
    output reg [255:0] data_outD,
    input [31:0] read_addrD,
    input [31:0] write_addrD,
    
    // Port E (32-bit AXI port)
    input clkE,
    input Axi_enable,
    input write_enE,
    input read_enE,
    input [31:0] data_inE,
    output reg [31:0] data_outE,
    input [31:0] addrE
);
    // Memory array
    reg [31:0] mem [0:1023];
    ...
    // Port B logic (same structure as Port A)
    ...
    // Port C logic (same structure as Port A)
    ...
    // Port D logic (same structure as Port A)
    ...
    // Port E logic (32-bit AXI port)
    always @(posedge clkE) begin
        if (reset) begin
            data_outE <= 32'b0;
        end else if (Axi_enable) begin  // Only operate when AXI is enabled
            if (write_enE && !read_enE) begin
                if (addrE < 1024) begin  // Simple boundary check
                    mem[addrE] <= data_inE;
                end
            end else if (!write_enE && read_enE) begin
                if (addrE < 1024) begin  // Simple boundary check
                    data_outE <= mem[addrE];
                end else begin
                    data_outE <= 32'b0;
