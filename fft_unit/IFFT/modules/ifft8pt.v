module ifft8pt #(parameter N = 32) (
    ...
    parameter signed [N-1:0] w2r0 = 32'h3f800000, w2i0 = 32'h0;
    parameter signed [N-1:0] w4r0 = 32'h3f800000, w4i0 = 32'h0, w4r1 = 32'h0, w4i1 = 32'hbf800000;
    parameter signed [N-1:0] w8r0 = 32'h3f800000, w8i0 = 32'h0, w8r2 = 32'h0, w8i2 = 32'hbf800000;
    parameter signed [N-1:0] w8r1 = 32'h3f350481, w8i1 = 32'hbf350481;
    parameter signed [N-1:0] w8r3 = 32'hbf350481, w8i3 = 32'hbf350481;
    ...
    ifft_2pt #(N) ifft2_1 (.clk(clk), .x0_real(mr0), .x0_imag(mi0), .x1_real(mr4), .x1_imag(mi4),
                        .twiddle_real(w8r0), .twiddle_imag(w8i0),
                        .X0_real(Xr0), .X0_imag(Xi0), .X1_real(Xr4), .X1_imag(Xi4));

    ifft_2pt #(N) ifft2_2 (.clk(clk), .x0_real(mr1), .x0_imag(mi1), .x1_real(mr5), .x1_imag(mi5),
                        .twiddle_real(w8r1), .twiddle_imag(w8i1),
                        .X0_real(Xr1), .X0_imag(Xi1), .X1_real(Xr5), .X1_imag(Xi5));

    ifft_2pt #(N) ifft2_3 (.clk(clk), .x0_real(mr2), .x0_imag(mi2), .x1_real(mr6), .x1_imag(mi6),
                        .twiddle_real(w8r2), .twiddle_imag(w8i2),
                        .X0_real(Xr2), .X0_imag(Xi2), .X1_real(Xr6), .X1_imag(Xi6));

    ifft_2pt #(N) ifft2_4 (.clk(clk), .x0_real(mr3), .x0_imag(mi3), .x1_real(mr7), .x1_imag(mi7),
